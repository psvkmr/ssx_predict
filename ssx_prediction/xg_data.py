...
class XgDataset:
    ...
    def filter_xg_dataset(self, xg_data=None, season_start_years=None, list_of_leagues=None):
        
        """Fitlers xG dataset provided by season year(s) and league name(s)
        
        Args:
            xg_data: provide xg data file if not taking from method import (Optional)
        ...
        # set defaults
        if season_start_years is None:
            season_start_years = [2017, 2018, 2019, 2020, 2021]

        if list_of_leagues is None:
            list_of_leagues = ['Barclays Premier League', 'English League Championship', 'UEFA Champions League',
    ...
    def get_team_xgs(self, season_start_years=None, list_of_leagues=None):
        ...
        # add default season start years
        if season_start_years is None:
...
if __name__ == '__main__':
    xg_obj = XgDataset(download_xg_data())
